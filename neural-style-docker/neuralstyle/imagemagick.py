...
def choptiles(imfile, xtiles, ytiles, tileoverlap, outname):
    """Chops an image file into a geometry of overlapping tiles. Returns ordered list of generated tiles image files"""
    command = 'convert %s -crop %dx%d+%d+%d@ +repage +adjoin %s_%%d.png' % (imfile, xtiles, ytiles, tileoverlap, tileoverlap, outname)
...
def feather(imfile, outname):
    ...
    command = 'convert %s -alpha set -virtual-pixel transparent -channel A -morphology Distance Euclidean:1,50\! +channel %s' % (imfile, outname)
...
def mergealpha(rgbfile, alphafile, resfile):
    ...
    if shape(rgbfile) != shape(alphafile):
        raise ValueError("Cant merge RGB and alpha images of differing sizes: %s vs %s" % (str(shape(rgbfile)), str(shape(alphafile))))
